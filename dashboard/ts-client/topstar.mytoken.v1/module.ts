...
export const txClient = ({ signer, prefix, addr }: TxClientOptions = { addr: "http://localhost:26657", prefix: "cosmos" }) => {
  ...
  return {
		...
		msgUpdateParams({ value }: msgUpdateParamsParams): EncodeObject {
			try {
				return { typeUrl: "/topstar.mytoken.v1.MsgUpdateParams", value: MsgUpdateParams.fromPartial( value ) }  
			} catch (e: any) {
		...
		msgMint({ value }: msgMintParams): EncodeObject {
			try {
				return { typeUrl: "/topstar.mytoken.v1.MsgMint", value: MsgMint.fromPartial( value ) }  
			} catch (e: any) {
...
const IgntModule = (test: IgniteClient) => {
	return {
		module: {
			TopstarMytokenV_1: new SDKModule(test)
		},
		registry: msgTypes
  }
}
